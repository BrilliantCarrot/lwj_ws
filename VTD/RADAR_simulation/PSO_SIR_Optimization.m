function optimal_path = PSO_SIR_Optimization(radar_pos, X, Y, Z, RADAR)
    ...
    for i = 1:num_particles
        % Calculate the corresponding z value for the (x, y) coordinate and add 100
        x = particles(i, 1);
        y = particles(i, 2);
    ...
    for iter = 1:max_iter
        for i = 1:num_particles
            ...
            current_score = find_sir(radar_pos, particles(i,:),X,Y,Z,RADAR);
...
function z = calculate_Z(x, y, X, Y, Z)
    % Find the corresponding z value for the given x and y coordinates
    [~, ix] = min(abs(X(1, :) - x));
    [~, iy] = min(abs(Y(:, 1) - y));
    z = Z(iy, ix);
end
